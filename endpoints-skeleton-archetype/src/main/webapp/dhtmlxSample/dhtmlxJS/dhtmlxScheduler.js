...
window.dhtmlXScheduler = window.scheduler = {
	...
	post: function (e, t, i) {
		...
		return a.async = arguments.length < 4,
		a.waitCall = i,
		a.loadXML(e, !0, t),
		a
	},
...
dtmlXMLLoaderObject.prototype.waitLoadFunction = function (e) {
	...
	a ? (this.xmlDoc.setRequestHeader("User-Agent", "dhtmlxRPC v0.1 (" + navigator.userAgent + ")"), this.xmlDoc.setRequestHeader("Content-type", "text/xml")) : t && this.xmlDoc.setRequestHeader("Content-type", "application/x-www-form-urlencoded"),
	...
	if (!((new Date).valueOf() - t.downtime < 100)) {
		...
		if (t.dragNode.parentNode != window.document.body && t.gldragNode) {
			...
			if (i.pWindow && i.pWindow.dhtmlDragAndDrop.lastLanding && i.pWindow.dhtmlDragAndDrop.lastLanding.dragLanding._dragOut(i.pWindow.dhtmlDragAndDrop.lastLanding), _isIE) {
				...
				r.innerHTML = t.dragNode.outerHTML,
				t.dragNode = r.childNodes[0];
			} else
	...
	if (!t) {
		...
		var a = i.lastLanding;
		i.lastLanding = null,
		a && a.dragLanding._drag(i.dragStartNode, i.dragStartObject, a, _isIE ? event.srcElement : e.target);
	}
	i.lastLanding = null,
...
dtmlXMLLoaderObject.prototype.doXPathOpera = function (e, t) {
	...
	return -1 != i[r - 1].indexOf("[") && (a = this._filterXPath(a, i[r - 1])),
	a
...
(), dhtmlx.attachEvent || dhtmlxEventable(dhtmlx), dataProcessor.prototype = {
	...
	setUpdated: function (e, t, i) {
		if (!this._silent_mode) {
			...
			r && "updated" == i && (i = r),
			t ? (this.set_invalid(e, !1), this.updatedRows[a] = e, this.obj.setUserData(e, this.action_param, i), this._in_progress[e] && (this._in_progress[e] = "wait")) : this.is_invalid(e) || (this.updatedRows.splice(a, 1), this.obj.setUserData(e, this.action_param, "")),
	...
	afterUpdateCallback: function (e, t, i, a) {
		...
		case "deleted":
			...
			this.callEvent("onAfterUpdate", [e, i, t, a])
		}
	...
	scheduler._waiAria = {
		...
		_eventCommonAttr: function (t, i) {
			i.setAttribute("aria-label", e(scheduler.templates.tooltip_text(t.start_date, t.end_date, t))),
			scheduler.config.readonly && i.setAttribute("aria-readonly", !0),
			t.$dataprocessor_class && i.setAttribute("aria-busy", !0),
			i.setAttribute("aria-selected", scheduler.getState().select_id == t.id ? "true" : "false")
		},
...
(), dhtmlxEventable(scheduler), scheduler._detachDomEvent = function (e, t, i) {
	...
	arguments.length > 3 && (e.style.left = a + "px",
		e.style.top = r + "px")
	...
	for (var e = this._obj.getElementsByTagName("DIV"), t = 0; t < e.length; t++) {
		...
		"string" != typeof r && a && !e[t].innerHTML && (r = a.split("_")[0]),
		r && (this._waiAria.labelAttr(e[t], r), e[t].innerHTML = r)
	}
	...
	if (!t) {
		...
		n > s.getHours() && (s.setHours(n), e = s.valueOf()),
		t = e.valueOf() + r
	}
	var d = new Date(t);
	...
	if (this._drag_mode) {
		...
		if (this._is_pos_changed(this._drag_pos, t)) {
			...
			if (this._drag_pos = t, this._drag_pos.has_moved = !0, "create" == this._drag_mode) {
				...
				this.addEvent(s, n, this.locale.labels.new_event, this._drag_id, t.fields),
			...
			if (scheduler.matrix && (d = scheduler.matrix[scheduler._mode]), d = d || {
				...
				if (i = l.start_date.valueOf(), a = l.end_date.valueOf(), this._table_view) {
					...
					c = new Date(_);
					a = _ + t.y * this.config.time_step * 6e4,
					a += 6e4 * (new Date(a).getTimezoneOffset() - c.getTimezoneOffset()),
				...
				if ("new-size" == this._drag_mode)
					if (a <= this._drag_start) {
						...
						i = a - (t.shift ? 0 : u),
						a = this._drag_start + (u || 6e4 * this.config.time_step)
					} else
						i = this._drag_start;
				else
			...
			if (!this._table_view && !scheduler.config.all_timed && (!scheduler._get_section_view() && t.x != this._get_event_sday({
				...
				a = scheduler.date.date_part(new Date(h)),
				a.setHours(this.config.last_hour),
				g = new Date(a - 1),
				"move" == this._drag_mode && (v = new Date(+a - f));
			}
	...
	for (var e = ["dhx_cal_next_button", "dhx_cal_prev_button", "dhx_cal_tab", "dhx_cal_today_button"], t = 0; t < e.length; t++)
		for (var i = this._els[e[t]], a = 0; i && a < i.length; a++) {
			...
			r && (s = this.locale.labels[r] || s),
			"dhx_cal_next_button" == e[t] ? s = this.locale.labels.next : "dhx_cal_prev_button" == e[t] && (s = this.locale.labels.prev),
			this._waiAria.headerButtonsAttributes(i[a], s || "")
		}
	...
	if (l)
		for (var o = 0; o < l.length; o++) {
			var h = l[o],
			_ = h.className;
			_ = _.replace(/ active/g, ""),
			h.getAttribute("name") == this._mode + "_tab" ? (_ += " active", this._waiAria.headerToggleState(h, !0)) : this._waiAria.headerToggleState(h, !1),
			h.className = _
		}
	this.update_view(),
	"number" == typeof n && (this._els[i][0].scrollTop = n)
	...
	r = r || 0;
	var s = document.createElement("DIV");
	s.className = "dhx_scale_bar",
	this.templates[this._mode + "_scalex_class"] && (s.className += " " + this.templates[this._mode + "_scalex_class"](i));
	var n = this._cols[e] - 1;
	"month" == this._mode && 0 === e && this.config.left_border && (s.className += " dhx_scale_bar_border", t += 1),
	this.set_xy(s, n, this.xy.scale_height - 2, t, r);
	var d = this.templates[this._mode + "_scale_date"](i, this._mode);
	s.innerHTML = d,
	this._waiAria.dayHeaderAttr(s, d),
	a.appendChild(s)
	...
	for (var o = 0; d > o; o++) {
		if (this._ignores[o] || this._render_x_header(o, l, n, e), !this._table_view) {
			...
			_ = "dhx_scale_holder";
			n.valueOf() == r.valueOf() && (_ = "dhx_scale_holder_now"),
			this._ignores_detected && this._ignores[o] && (_ += " dhx_scale_ignore"),
			h.className = _ + " " + this.templates.week_date_class(n, r),
	...
	if (this.templates[this._mode + "_date"]) {
		...
		r = s = this._table_view ? scheduler.date.week_start(n) : n,
		this._min_date = r;
		...
		if (this._els.dhx_cal_date[0].innerHTML = l, this._waiAria.navBarDateAttr(this._els.dhx_cal_date[0], l), this._max_date = d, scheduler._render_scales(e, t), this._table_view)
			...
			this.set_xy(c, Math.max(this._colsS[this._colsS.col_length] + this.xy.scroll_width - 2, 0), 0, 0, _),
	...
	a = a || Math.ceil(Math.round((s.valueOf() - i.valueOf()) / 864e5) / 7);
	for (var l = [], o = 0; 7 >= o; o++) {
		var h = (this._cols[o] || 0) - 1;
		0 === o && this.config.left_border && (h -= 1),
		l[o] = h + "px"
	}
	var _ = 0,
	...
	for (var g = [], o = 0; a > o; o++) {
		...
		for (var f = Math.max(r(o) - scheduler.xy.month_head_height, 0), m = 0; 7 > m; m++) {
			...
			v.appendChild(p);
			var b = "";
			t > i ? b = "dhx_before" : i >= s ? b = "dhx_after" : i.valueOf() == d.valueOf() && (b = "dhx_now"),
	...
	return this._colsS.height = n + this.xy.month_head_height,
	this._colsS.heights = [],
	scheduler._render_month_scale(e, t, i, a)
	...
	var s = e;
	1 != arguments.length && (s = r || {}, s.start_date = e, s.end_date = t, s.text = i, s.id = a),
	s.id = s.id || scheduler.uid(),
	s.text = s.text || "",
	...
	s.start_date.valueOf() == s.end_date.valueOf() && s.end_date.setTime(s.end_date.valueOf() + n),
	s._timed = this.isOneDayEvent(s);
	var d = !this._events[s.id];
	return this._events[s.id] = s,
	this.event_updated(s),
	this._loading || this.callEvent(d ? "onEventAdded" : "onEventChanged", [s.id, s]),
	s.id
...
(), scheduler.event_updated = function (e, t) {
	...
	for (var t = this._rendered_location, i = this._lame_clone(e), a = e._sday; a < e._eday; a++) {
		...
		s = r.add(s, i._sweek, "week"),
		s = r.add(s, i._sday, "day");
		...
		o.style.left = d.x + "px",
		o.style.width = l + "px",
		o.innerHTML = scheduler.templates.month_events_link(s, n),
...
scheduler.render_data = function (e, t) {
	...
	if (e = this._table_view ? this._pre_render_events_table(e, t) : this._pre_render_events_line(e, t), this._table_view)
		if (t)
			...
			if (n.rows) {
				...
				f = v,
				m = v + "px";
				this.config.multi_day_height_limit && (f = Math.min(v, this.config.multi_day_height_limit), m = f + "px"),
				s.style.top = this._els.dhx_cal_navline[0].offsetHeight + this._els.dhx_cal_header[0].offsetHeight + f + "px",
				s.style.height = this._obj.offsetHeight - parseInt(s.style.top, 10) - (this.xy.margin_top || 0) + "px";
				var p = this._els.dhx_multi_day[0];
				p.style.height = m,
				p.style.visibility = -1 == r[0] ? "hidden" : "visible";
				...
				this._dy_shift = (r[0] + 1) * i,
				this.config.multi_day_height_limit && (this._dy_shift = Math.min(this.config.multi_day_height_limit, this._dy_shift)),
				r[0] = 0,
				f != v && (s.style.top = parseInt(s.style.top) + 2 + "px", p.style.overflowY = "auto", b.style.position = "fixed",
			...
			if (i[s._sday] || (i[s._sday] = []), !t) {
				...
				if (h.length && (h[h.length - 1]._inner = !0), !g)
					if (h.length)
						...
						s._sorder = 0;
				h.splice(u, u == h.length ? 0 : 1, s),
				h.length > (h.max_count || 0) ? (h.max_count = h.length, s._count = h.length) : s._count = s._count ? s._count : 1
			}
	...
	for (var i, a = [], r = [[], [], [], [], [], [], []], s = this._colsS.heights, n = this._cols.length, d = {}, l = 0; l < e.length; l++) {
		...
		if (o._sday = g % n, !this._ignores[o._sday] || !o._timed) {
			...
			o._eday = v % n || n,
			o._length = v - g,
			o._sweek = Math.floor((this._correct_shift(c.valueOf(), 1) - this._min_date.valueOf()) / (864e5 * n));
			var f,
			m = r[o._sweek];
	...
	if (!(e._sday < 0)) {
		...
		if (a) {
			...
			if (this._waiAria.eventAttr(e, g), this._rendered.push(g), a.appendChild(g), h = h + parseInt(a.style.left, 10) + i, this._edit_id == e.id) {
				...
				this._quirks7 && (f.firstChild.style.height = n - 12 + "px"),
	...
	return l && (p = "dhx_resize_denied " + p),
	m += '<div class="' + p + '" style=" width:' + (a - 8) + "px;" + (l ? " margin-top:-1px;" : "") + u + g + '" ></div>',
	f.innerHTML = m,
	f
	...
	if (s) {
		...
		_ && (n += "<div class='dhx_event_resize dhx_event_resize_start'></div>"),
	...
	if (!e.parentNode) {
		...
		a.indexOf("dhx_scale_holder") < 0 && i.previousSibling && (i = i.previousSibling),
		a = scheduler._getClassName(i),
		i && 0 === a.indexOf("dhx_scale_holder") && i.appendChild(e)
	}
...
scheduler.ical = {
	...
	parse_param: function (e, t) {
		...
		if (-1 != i) {
			...
			"summary" == a ? a = "text" : "dtstart" == a ? (a = "start_date", r = this.parse_date(r, 0, 0)) : "dtend" == a && (a = "end_date", r = this.parse_date(r, 0, 0)),
			t[a] = r
		}
	},
...
scheduler.formSection = function (e) {
	...
	select: {
		...
		set_value: function (e, t, i, a) {
			var r = e.firstChild;
			!r._dhx_onchange && a.onchange && (r.onchange = a.onchange, r._dhx_onchange = !0),
			"undefined" == typeof t && (t = (r.options[0] || {}).value),
			r.value = t || ""
		},
		get_value: function (e, t) {
			...
			for (var n = "", d = 0; d < t.length; d++) {
				...
				case "%H:%i":
					...
					for (e._time_values = []; r > _; ) {
						...
						e._time_values.push(_),
						a.setTime(a.valueOf() + 60 * this.config.time_step * 1e3);
		...
		set_value: function (e, t, i, a) {
			...
			if (d.full_day) {
				...
				_.onclick = function () {
					if (_.checked) {
						var t = {};
						scheduler.form_blocks.time.get_value(e, t, a),
						s = scheduler.date.date_part(t.start_date),
						n = scheduler.date.date_part(t.end_date),
						(+n == +s || +n >= +s && (0 !== i.end_date.getHours() || 0 !== i.end_date.getMinutes())) && (n = scheduler.date.add(n, 1, "day"));
					} else
						s = null, n = null;
					l[o[0]].disabled = _.checked,
					l[o[0] + l.length / 2].disabled = _.checked,
					r(l, 0, s || i.start_date),
					r(l, 4, n || i.end_date)
				}
			}
			if (d.auto_end_date && d.event_duration)
				for (var c = function () {
					s = new Date(l[o[3]].value, l[o[2]].value, l[o[1]].value, 0, l[o[0]].value),
					n = new Date(s.getTime() + 60 * scheduler.config.event_duration * 1e3),
					r(l, 4, n)
				}, u = 0; 4 > u; u++)
					l[u].onchange = c;
			r(l, 0, i.start_date),
	...
	if (e) {
		...
		t ? e.style.top = Math.round(t + Math.max((a - e.offsetHeight) / 2, 0)) + "px" : e.style.top = Math.round(Math.max((a - e.offsetHeight) / 2, 0) + 9) + "px",
		...
		if (t && i)
			...
			default:
				if (t.getAttribute("dhx_button"))
					...
					s,
					n;
					-1 != i.indexOf("dhx_custom_button") && (-1 != i.indexOf("dhx_custom_button_") ? (r = t.parentNode.getAttribute("index"), n = t.parentNode.parentNode) : (r = t.getAttribute("index"), n = t.parentNode, t = t.firstChild)),
					r && (s = scheduler.form_blocks[scheduler.config.lightbox.sections[r].type],
	...
	if (!this._lightbox) {
		...
		for (var r = 0; r < s.length; r++) {
			...
			if (n) {
				...
				"string" != typeof l && (l = s[r].name),
				t += "<div id='" + s[r].id + "' class='dhx_cal_lsection'>" + d + "<label>" + l + "</label></div>" + n.render.call(this, s[r]),
				t += "</div>"
			}
		}
	...
	a(this._els.dhx_cal_data[0], "scroll", n),
	a(this._els.dhx_cal_data[0], "touchcancel", n),
	a(this._els.dhx_cal_data[0], "contextmenu", function (e) {
		...
		scheduler.attachEvent("onTemplatesReady", function () {
			...
			if (scheduler.templates.month_day = function (i) {
				if ("month" == this._mode) {
					...
					+i == +scheduler.date.date_part(this._currentDate()) && (a = scheduler.locale.labels.dhx_cal_today_button + " " + a),
					a
				}
				...
				for (var i = scheduler._els.dhx_cal_navline[0].getElementsByTagName("div"), a = null, r = 211, s = 0; s < i.length; s++) {
					...
					if (d)
						...
						case "month_tab":
							n.style.left = "136px",
							n.className += " dhx_cal_tab_last";
							break;
						default:
							n.style.left = r + "px",
							n.className += " dhx_cal_tab_standalone",
							r = r + 14 + n.offsetWidth
						}
					else
